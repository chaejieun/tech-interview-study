...
- CPU-bound 작업은 주로 복잡한 연산이 많은 작업으로, CPU 성능이 병목이 됩니다. 예를 들어 대규모 수학 계산이나 이미지 처리 등이 해당됩니다.
반면 I/O-bound 작업은 외부 시스템과의 데이터 송수신이 병목인 작업입니다. 예를 들어 DB 쿼리, 파일 읽기, REST API 호출 등이 있으며, 이 경우 비동기 처리로 대기 시간을 줄일 수 있습니다.
실제로 저는 I/O-bound 상황에서는 Spring의 @Async나 WebClient를 사용하고, CPU-bound 연산은 병렬 스트림이나 멀티코어 환경을 활용해 처리한 경험이 있습니다.


# 시스템 콜이란 무엇인가요?
- 시스템 콜은 사용자 애플리케이션이 운영체제의 자원이나 기능을 사용하기 위해 요청하는 인터페이스입니다.
예를 들어, 파일 입출력, 프로세스 관리, 네트워크 통신 등이 모두 시스템 콜을 통해 이루어집니다.
시스템 콜은 사용자 모드와 커널 모드 간 권한 전환이 수반되어 일반 함수 호출보다 비용이 크므로, 성능 최적화 시 고려해야 합니다.


# 사용자(User) 모드와 커널(Kernel) 모드의 차이를 설명해 주세요.

## 사용자 모드(User Mode)란?
- **일반 애플리케이션(사용자 프로그램)이 실행되는 모드**
- CPU가 제한된 권한만 갖고 실행됨
- 하드웨어나 중요한 자원에 직접 접근 불가능
- 오류가 나도 시스템 전체에 영향을 적게 미침

## 커널 모드(Kernel Mode)란?
- **운영체제 커널이 실행되는 모드**
- CPU가 모든 권한(특권 모드)을 갖고 실행됨
- 하드웨어 직접 제어, 메모리 관리, 프로세스 관리, 파일 시스템 등 핵심 작업 수행
- 커널 모드에서 발생한 오류는 시스템 전체에 치명적일 수 있음
