...
# 📅 2025/06/18
# 파이프라이닝에 대해서 설명해주세요
- 파이프라이닝은 컴퓨터 과학에서 **여러 작업을 동시에 또는 겹쳐서 실행하여 전체 처리량을 높이는 기술**을 말한다. 마치 공장의 조립 라인처럼, 하나의 작업을 여러 단계로 나누고 각 단계가 동시에 다른 작업의 다른 단계를 처리하도록 함으로써 유혹 시간을 줄이고 효율성을 극대화 한다.

## 왜 파이프라이닝을 사용할까요?
- 단일 프로세서에서 한 번에 하나의 명령어만 처리한다면, 특정 단계가 완료될 때까지 다음 단계는 기다려야 합니다. 이는 자원 낭비로 이어지고 전체적인 처리 속도를 저하시킵니다. 파이프라이닝은 이러한 비효율성을 개선하여 CPU의 활용도를 높이고 프로그램 실행 속도를 향상 시킨다.
