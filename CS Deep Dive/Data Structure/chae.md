...
- Retrieval에서 따온 이름으로, 문자열 검색에 최적화 되어있다. 주로 자동 완성, 철자 검사, 사전 검색, IP 라우팅 등 문자열 기반의 작업에서 강력한 성능을 발휘한다.

## 트라이의 기본 구조
- 루트 노드(Root Node) : 트라이의 시작점이며, 보통 빈 문자열("")을 나타낸다
- 노드(Node) : 각 노드는 하나의 문자를 나타낸다
- 간선(Edge) : 한 노드에서 다음 노드로 이어지는 간선은 특정 문자를 의미. 
- 자식 노드(Children Nodes) : 각 노드는 다음 문자를 나타내는 여러 개의 자식 노드를 가질 수 있다. 일반적으로 배열 또는 해시 맵으로 자식 노드를 관리한다
- 마지막 문자 표시(End of Word Mark): 각 노드에는 해당 노드까지의 경로가 완전한 단어인지 여부를 나타내는 플래그가 있다

## 트라이의 단점
- 메모리 사용량: 각 노드가 자식 노드에 대한 포인터(또는 배열)를 가지고 있어야 하므로, 저장되는 문자열의 개수와 길이에 따라 상당한 메모리를 소비할 수 있습니다. 특히 알파벳 집합이 크거나 문자열 길이가 긴 경우 더욱 두드러집니다.
- 단어 수가 적을 때 비효율적: 저장할 단어의 수가 적을 때는 해시 테이블이나 정렬된 배열보다 오히려 성능이 좋지 않을 수 있습니다.

## 트라이의 활용 분야
- 자동 완성 (Autocomplete): 입력 중인 단어의 접두사를 기반으로 가능한 단어 목록을 제안합니다.
- 철자 검사 (Spell Checker): 사전에 등록된 단어와 비교하여 오타를 찾아내거나 수정 제안을 합니다.
- 사전 구현: 단어를 효율적으로 저장하고 검색합니다.
- IP 라우팅 (Longest Prefix Match): IP 주소의 가장 긴 일치 접두사를 찾아 경로를 결정합니다.
- 바이오인포매틱스: DNA/RNA 염기 서열 검색.
- 검색 엔진: 특정 키워드로 시작하는 문서들을 빠르게 찾을 때 활용될 수 있습니다