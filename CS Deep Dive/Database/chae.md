...
# 📅 2025/06/20
# 레디스(Redis)란?
- 레디스는 "REmote Dictionary Sever"의 약자로, **오픈 소스 인메모리(in-memory) 데이터 구조 스토어**이다. 일반적으로 데이터베이스, 캐시, 메시지 브로커로 사용. 
- 디스크에 저장하는 전통적인 데이터베이스와 달리, 데이터를 주로 **메모리에 저장**하여 매우 빠른 읽기/쓰기 성능을 제공하는 것이 가장 큰 특징.

## 레디스의 주요 특징
1. 인메모리 데이터 스토어 : 모든 데이터가 RAM에 저장되므로, 하드 디스크에 접근하는 것보다 훨씬 빠르게 데이터를 처리할 수 있다
2. 다양한 데이터 구조 지원 : 단순한 문자열 뿐만 아니라 복잡한 데이터 구조를 기본적으로 지원(해시, 리스트, 세트, 비트맵..)
3. 단일 스레드 아키테처 : 레디스 서버 자체는 단일 스레드로 동작하여 동시성 제어에 대한 복잡성을 줄이고, 컨텍스트 스위칭 오버헤드를 없애 성능을 극대화 한다.
4. 높은 성능 : 메모리 기반, 최적화된 자료 구조, 단일 스레드 아키텍처 덕분에 초당 수십만 건의 작업을 처리할 수 있는 매우 빠른 성능을 자랑
5. 다양한 언어 지원

## 레디스의 주요 사용 사례

- 캐싱 (Caching): 가장 일반적인 용도로, 데이터베이스 쿼리 결과나 자주 접근하는 데이터를 캐싱하여 애플리케이션의 응답 속도를 향상시킵니다.
- 세션 관리 (Session Management): 웹 애플리케이션에서 사용자 세션 정보를 저장하는 데 사용됩니다.
- 실시간 분석 (Real-time Analytics): 실시간으로 발생하는 이벤트 데이터를 처리하고 분석하는 데 활용됩니다. (예: 게임 리더보드, 실시간 랭킹)
- 메시지 브로커 / 큐 (Message Broker / Queue): Pub/Sub(발행/구독) 기능을 통해 메시지 큐 시스템을 구현하거나, 리스트(List)를 이용하여 작업 큐를 구성합니다.
- 락 (Lock) 시스템: 분산 환경에서 여러 서버가 공유 자원에 동시에 접근하는 것을 방지하기 위한 분산 락(distributed lock) 구현에 사용됩니다.
- 게시판 / 뉴스피드: 실시간으로 업데이트되는 피드를 구성하는 데 사용될 수 있습니다.
- 추천 시스템: 사용자 활동 데이터를 기반으로 추천 목록을 빠르게 생성하고 제공합니다.
...
- 엘라스틱서치는 분산형 RESTful 검색 및 분석 엔진이다. Apache Lucene을 기반으로 구축되었으며, 방대한 양의 정형 및 비정형 데이터를 실시간으로 저장, 검색, 분석할 수 있도록 설계되었다.
- 주로 **로그 및 이벤트 데이터 분석, 전문 검색(Full-text Search), 보안 정보 및 이벤트 관리(SIEM), 애플리케이션 성능 모니터링(APM)**등 다양한 용도로 사용된다.

## 엘라스틱서치의 주요 특징
1. 분산형 아키텍처(Distributed Architecture)
- 여러 서버(노드)에 데이터를 분산 저장하고 처리할 수 있어 **수평 확장성(Horizontal Scalability)**이 뛰어나다. 데이터 양이 늘어나도 노드를 추가하여 쉽게 확장할 수 있다
- 노드 장애 발생 시에도 데이터의 가용성과 내결함성을 보장
2. RESTful API
- HTTP 기반의 RESTful API를 통해 데이터를 쉽게 색인(Index)하고 검색, 업데이트, 삭제할 수 있다. 
3. 전문 검색(Full-text Search)
- 텍스트 데이터를 효율적으로 색인하고, 관련성 높은 검색 결과를 빠르게 반환하는 강력한 전문 검색 기능 제공
4. 실시간 데이터 처리
5. 다양한 분석 기능

## 엘라스틱서치의 핵심 개념
- 인덱스 (Index): 관계형 데이터베이스의 '데이터베이스'와 유사한 개념으로, 논리적으로 관련된 문서들의 컬렉션입니다.
- 타입 (Type): (Elasticsearch 6.x 버전부터 deprecated되었고, 7.x부터는 완전히 제거됨) 과거에는 인덱스 내에서 서로 다른 데이터 구조를 가진 문서들을 구분하는 데 사용되었습니다. 현재는 단일 인덱스 내에서 모든 문서가 하나의 타입으로 간주되거나, 여러 인덱스를 사용하는 방식으로 대체됩니다.
- 문서 (Document): 엘라스틱서치에 저장되는 가장 기본적인 단위로, JSON 형식으로 표현됩니다. 관계형 데이터베이스의 '행(Row)'과 유사합니다.
