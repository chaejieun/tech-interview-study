...
# LocalStorage와 SessionStorage의 차이점은 무엇인가요? 서버 입장에서 주의할 점은?
# LocalStorage와 SessionStorage의 차이점

| 특징         | LocalStorage                                   | SessionStorage                                   |
| :----------- | :--------------------------------------------- | :----------------------------------------------- |
| **만료 기간** | 영구적 (사용자가 명시적으로 삭제하거나, 브라우저 캐시를 지우기 전까지 유지) | 세션 기반 (브라우저 탭/창을 닫으면 데이터 삭제) |
| **접근 범위** | 동일 도메인의 모든 탭/창에서 접근 가능        | 현재 탭/창 내에서만 접근 가능 (새로운 탭/창에서는 접근 불가) |
| **데이터 공유** | 여러 탭/창 간에 데이터 공유 가능              | 단일 탭/창 내에서만 데이터 공유 가능            |
| **데이터 크기** | 일반적으로 5MB ~ 10MB (브라우저마다 다름)    | 일반적으로 5MB ~ 10MB (브라우저마다 다름)        |
| **용도** | 사용자 설정, 로그인 상태 유지 (만료 없는 정보), 오프라인 데이터 등 | 임시 데이터, 단일 세션 동안의 사용자 입력, 일회성 데이터 등 |

- LocalStorage와 SessionStorage는 클라이언트 측에서 편리하게 데이터를 저장할 수 있는 강력한 도구이지만, 서버 입장에서는 보안, 데이터 무결성, 세션 관리 측면에서 신중하게 접근하고 설계해야 한다. 민감한 정보는 절대 클라이언트 측에 저장하지 말고, 모든 중요한 로직은 서버에서 처리 및 검증하는 것을 원칙으로 삼아야 합니다.


# 📅 2025/07/01
# CORS(Cross-Origin Resource Sharing)란 무엇인가요? 왜 필요한가요?
- **CORS(Cross-Origin Resource Sharing, 교차 출원 자원 공유)**는 웹 브라우저에서 실행되는 스크립트가 다른 "Origin"의 자원에 접근할 수 있도록 허용하는 메커니즘입니다.
- CORS는 서버와 클라이언트 간의 HTTP 헤더 교환을 통해 작동한다. 클라이언트(브라우저)는 요청 헤더에 자신의 Origin을 포함하여 서버에 전송하고, 서버는 응답 헤더에 `Access-Control-Allow-Origin`과 같은 CORS 관련 헤더를 포함하여 해당 요청을 허용할 지 여부를 브라우저에 알려준다

## CORS는 왜 필요한가?
- 웹 보안의 핵심적인 부분인 **동일 출원 정책(SOP)**의 제약을 완화하면서도, 동시에 잠재적인 **보안 위협으로부터 사용자를 보호**하기 위해 필요하다.


# CORS 오류가 발생했을 때 어떻게 디버깅하고 해결하나요?

## CORS 오류 디버깅
1. 브라우저 개발자 도구 열기
2. 콘솔 탭 확인
3. 네트워크 탭 확인
4. Preflight Request 확인 

## CORS 오류 해결 방법
1. 서버 측 설정 
    - `Access-Control-Allow-Origin` 헤더 설정 : 서버는 요청을 보낸 클라이언트의 Origin을 허용해야 합니다.
    - `Access-Control-Allow-Methods` 헤더 설정 : 버가 허용하는 HTTP 메서드(GET, POST, PUT, DELETE 등)를 명시
    - `Access-Control-Allow-Headers` 헤더 설정 : 클라이언트가 요청에 포함할 수 있는 사용자 정의 헤더를 명시
    - `Access-Control-Allow-Credentials` 헤더 설정 : 클라이언트가 인증 정보(쿠키, HTTP 인증 헤더)를 함께 보낼 필요가 있을 때 사용
    - `Access-Control-Allow-Max-Age` 헤더 설정 : Preflight 요청의 결과를 캐시할 시간을 초 단위로 지정.

# CSRF(Cross-Site Request Forgery) 공격은 무엇이고, 어떻게 방어할 수 있나요?
- CSRF는 사용자가 로그인한 웹사이트에 대해 의도하지 않은 요청을 보내게 만드는 공격.
- 사용자가 방문한 다른 웹사이트나 이메일 등을 통해 악성 스크립트나 이미지를 삽입하여, 사용자의 브라우저가 정상적인 웹사이트에 특정 요청을 보내도록 유도한다. 
- **사용자가 해당 웹사이트에 로그인되어 있는 상태**여야 한다. 

## CSRF 공격 방어
1. CSRF 토큰 
    - 사용자가 웹 페이지를 요청할 때, 서버는 임의의 고유한 CSRF 토큰을 생성한다
    - 이 토큰을 사용자의 세션에 저장하고, 동시에 웹 페이지의 숨겨진 필드나 HTTP 헤더에 포함시켜 클라이언트에게 전달한다
2. SameSite 쿠키 속성 
    - 브라우저가 특정 쿠키를 '교차 출원'요청에 첨부할지 여부를 제어한다
3. Referer 검증 (제한적 사용)
