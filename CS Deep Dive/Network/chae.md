...
## JWT 인증과정
1. 사용자 로그인
2. 토큰 생성 및 발급 : 사용자의 인증 정보 확인 후, 사용자 정보가 담긴 페이로드와 함께 JWT를 생성. 이때 서버만 아는 **비밀 키(Secret Key)**를 사용하여 시그니처를 만들고, 생성된 JWT를 클라이언트에 전송
3. 토큰 저장 : 클라이언트는 서버로부터 받은 JWT를 웹 브라우저의 로컬 스토리지, 세션 스토리지 또는 쿠키에 저장
4. 자원 접근 요청 : 클라이언트는 보호된 자원에 접근할 때마다, 저장해 둔 JWT를 HTTP 요청 헤더의 `Authorization` 필드에 담아 서버로 보낸다
5. 토큰 유효성 검증 : 서버는 클라이언트로부터 받은 JWT의 시그니처를 검증한다
    - 서버는 클라이언트가 보낸 토큰의 헤더와 페이로드를 비밀 키로 다시 해싱하여 새로운 시그니처를 생성
    - 이때 새로 생성된 시그니처와 토큰에 포함된 시그니처가 일치하는 지 확인
    - 일치한다면, 토큰이 위변조되지 않았다고 판단하고 페이로드의 정보를 사용하여 요청을 처리
    - 일치하지 않거나 토큰이 만료되면 인증 실패로 요청을 거부 

# OAuth에 대해서 간단히 설명해주세요
- Open Authorization의 약자로, 사용자가 자신의 **비밀벊로를 직접 공유하지 않고** 다른 애플리케이션(서비스)에 자신의 정보에 대한 접근 권한을 부여할 수 있도록 하는 개방형 **권한 부여(Authorization)** 프로토콜
- ex) 구글로 로그인, 카카오로 로그인과 같은 소셜 로그인 기능 

## OAuth의 핵심 역할
- 4가지 역할 간의 상호작용을 통해 이루어짐
1. 리소스 소유자(Resource Owner) : 데이터의 실제 소유자인 사용자
2. 클라이언트(Client) : 사용자의 정보에 접근하려는 제3자 애플리케이션
3. 리소스 서버(Resource Server) : 사용자의 보호된 정보(리소스)를 가지고 잇는 서버(ex:구글 서버)
4. 권한 서버(Authorization Server) : 사용자의 동의를 얻어 클라이언트에게 **접근 토큰(Access Token)**을 발급하는 서버 (ex:구글 로그인 페이지)


# JWT와 OAuth의 차이는 무엇이 있을까요?
- JWT는 **토큰의 형식** 이고, OAuth는 **토큰을 주고 받는 프로토콜(절차)**
### 🆚 JWT와 OAuth의 주요 차이점

| 구분 | JWT (JSON Web Token) | OAuth (Open Authorization) |
| :--- | :--- | :--- |
| **목적** | **정보 전송**을 위한 **토큰 형식** | **권한 부여**를 위한 **프레임워크/프로토콜** |
| **역할** | 사용자 정보나 권한을 담아 당사자 간에 전달하는 **토큰 자체** | 사용자 동의를 얻어 토큰을 발급하고, 제3자 애플리케이션이 리소스에 접근하도록 **허용하는 절차** |
| **활용** | 서버 간의 **인증 및 정보 교환**에 주로 사용 (API 인증, 마이크로서비스 등) | 소셜 로그인과 같이 제3자 서비스에 **접근 권한을 위임**하는 데 사용 |
| **상태 관리** | **무상태(Stateless)**: 토큰 자체가 모든 정보를 담고 있어 서버가 세션 정보를 유지할 필요가 없음. | **상태 관리(Stateful)**: 여러 단계의 상호작용과 서버 상태를 통해 토큰을 발급하고 관리함. |
| **관계** | **OAuth의 접근 토큰(Access Token)으로 JWT를 사용**할 수 있습니다. | OAuth는 JWT를 사용하여 **토큰을 발급하고 검증하는 하나의 방법**으로 활용할 수 있습니다. |